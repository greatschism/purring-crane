...
(function() {
  ...
  var holdingName = win._holdingName;
  var holdingsList = [];
  var holdingInstructionsText = (holdingsList.length) ? 
                                "Or edit an existing holding:" : 
                                "You have no holdings added yet. Use the above search field to find your first holding."
  ...
  */
  ...
  ConnectToHTButton.addEventListener("click", function() {
    ...
    var SuccessWindow = Titanium.UI.createWindow(
    ...
    var ConnectToHTButton = Titanium.UI.createButton(
      win.HT.combine(styles.LargeBlueButton, {
    ...
    CloseButton.addEventListener("click", function() {SuccessWindow.close();});
    
    SuccessWindow.open({modal: true});
  });
